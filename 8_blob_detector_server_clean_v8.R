...
input <- Sys.getenv('input') # input <- "054112895030_01"
...
Path_in <- paste(root, "4_categ", sep="/")
Path_out <- paste(root, "5_blob_detection", input, sep="/")
...
Satellite <- metadata[2,2]
Band_nr <- metadata[3,2]
Study_area <- metadata[9,2]

Path_in <- paste(Path_in, Study_area, sep="/")
...
if(WriteLog){
	setwd(Path_out)
	sink(file=paste("combine_tiles_allblobs_",ms.imagefn,".txt",sep=""))
	cat(paste(Sys.time(),"starting processing of image id=",ms.imagefn,"\n",sep=" "))
}
...
for(iy in iy_arr){
	...
	setwd(Path_out)
	save(Blobs, file=paste("nonovl_blobs_image_",input,"_tx_",ix,"_ty_",iy,".RData",sep=""))

	if(WriteLog) cat(as.character(Sys.time())," tile ",ix," ",iy," successfully processed",nrow(Blobs)," blobs detected","\n")

	if((ix==1)&(iy==1)) All_blobs <- Blobs else All_blobs <- rbind(All_blobs,Blobs)
}
...
save(Blobs, file=paste("all_blobs_image_",input,".RData",sep=""))
